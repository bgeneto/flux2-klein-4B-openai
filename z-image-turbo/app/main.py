...
class Settings(BaseSettings):
    ...
    vae_path: Optional[Path] = Path("/models/ae-f16.gguf")
...
REQUEST_COUNTER = Counter(
    "z_image_api_requests_total",
    "Total API requests",
    ["endpoint", "method", "status"],
)
JOB_COUNTER = Counter("z_image_api_jobs_total", "Total jobs", ["status"])
JOB_DURATION = Histogram(
    "z_image_api_job_duration_seconds", "Image generation job duration"
)
QUEUE_DEPTH = Gauge("z_image_api_queue_depth", "Current job queue depth")
RUNNING_JOBS = Gauge("z_image_api_running_jobs", "Currently running jobs")
...
class SDServerManager:
    ...
    def _build_command(self) -> list[str]:
        ...
        if settings.enable_vae_conv_direct:
            cmd.append("--vae-conv-direct")

        if settings.enable_vae_tiling:
            cmd.append("--vae-tiling")

        if settings.enable_clip_on_cpu:
...
app = FastAPI(title="Z-Image Turbo stable-diffusion.cpp API", version="1.0.0")
